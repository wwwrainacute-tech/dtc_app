...
import { Icon, FieldRenderer } from './fields.jsx';
import { DTCStore } from './store.js';
import { DTC } from './schemas.js';
import { useTweaks, TweaksPanel, TweakSection, TweakColor, TweakToggle, TweakRadio } from './tweaks-panel.jsx';
/* ============================================================
...
/* ===================== Caregiver root ===================== */
function CaregiverApp({ embedded, page = "today", onNav }) {
  ...
  return (
    <div className="screen caregiver-screen">
      {page === "today" && <TodayTab onStart={startForm} submissions={submissions} />}
      {page === "forms" && <FormsTab onStart={startForm} />}
      {page === "records" && <RecordsTab submissions={submissions} onOpen={(s) => setViewing(s)} />}
      ...
      {done ? <DoneScreen schema={done.schema} client={done.client} onClose={() => { setDone(null); if (onNav) onNav("today"); }} onViewRecords={() => { setDone(null); if (onNav) onNav("records"); }} /> : null}
...
export { CaregiverApp, PdfPreview, ScoreBlock, RecordViewer, displayValue, fmtDate, getSchema };

// Standalone render removed — mounted by App.tsx router
