...
function AutofillChip({ field, ctx, value, onApply }) {
  const suggested = window.DTC.resolveAutofill(field, ctx);
...
function FieldRenderer({ field, value, onChange, ctx, invalid, score }) {
  ...
  switch (field.type) {
    ...
    case "table":
      ...
      break;
    case "signature": {
      const suggest = field.autofill ? window.DTC.resolveAutofill(field, ctx) : null;
...
window.DTCFields = { Icon, FieldRenderer, SignaturePad };
